...
async function main() {
  ...
  console.log('\n🚀 DEPLOYING BOTSTATE CONTRACTS TO BOT CHAIN MAINNET');
  ...
  const prContract = await prFactory.deploy({ gasPrice, nonce: nonce++ });
  ...
  const rwaContract = await rwaFactory.deploy({ gasPrice, nonce: nonce++ });
  ...
  const mpContract = await mpFactory.deploy({ gasPrice, nonce: nonce++ });
  ...
  const aalContract = await aalFactory.deploy(wallet.address, { gasPrice, nonce: nonce++ });
  ...
  console.log('\n🎉 ALL 4 CONTRACTS DEPLOYED & CONFIRMED ON BOT CHAIN MAINNET!');
  console.log('====================================================');
